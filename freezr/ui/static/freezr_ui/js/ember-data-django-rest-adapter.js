...
DS.DjangoRESTSerializer = DS.RESTSerializer.extend({
    ...
    extractDjangoPayload: function(store, type, payload) {
        type.eachRelationship(function(key, relationship){
            if (Ember.isNone(payload[key]))
                return;
            console.log("key", key, "payload[key]", payload[key], "typeof", typeof(payload[key]))
            if (relationship.kind === 'hasMany') {
                var payloadType = typeof(payload[key][0]);

                if (payloadType !== 'number' &&
                    payLoadType !== 'string' &&
                    payload[key].constructor.name === 'Array') {
                    if (payload[key].length > 0) {
                        var ids = payload[key].mapBy('id'); //todo find pk (not always id)
                        this.pushArrayPayload(store, relationship.type,
                                              payload[key]);
                        payload[key] = ids;
                    }
                }
            } else if (relationship.kind == 'belongsTo') {
                var payloadType = typeof(payload[key]);
                if (payloadType === 'object') {
                    var id = payload[key].id;
                    this.pushSinglePayload(store, relationship.type,
                                           payload[key]);
                    payload[key] = id;
    ...
    */
	...
	//payload = type.find(payload)
	payload = store.find(type, payload)
